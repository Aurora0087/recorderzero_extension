...
import { MoreHorizontalIcon, Edit3Icon, PencilIcon, TrashIcon, AlertTriangleIcon, Download } from 'lucide-react';
...
function VideoCard({ video }: {
    ...
    const [selectedFormat, setSelectedFormat] = useState<'mp4' | 'webm' | 'mov' | 'gif'>('mp4')
    ...
    function renameVideo(newName: string) {
        if (newName.trim()) {
    ...
    function gotoEditPage() {
        const videoEditerPath = browser.runtime.getURL(`/video-edited.html?vi=${video.id}`);
    ...
    return (
        <>
            <Card
                ...
                <div className='px-4 flex justify-between w-full gap-2 items-start'>
                    <div className="space-y-1 flex-1 min-w-0">
                        <CardTitle className="text-lg line-clamp-1 text-foreground">{video.name}</CardTitle>
                        <span className="text-xs opacity-60 font-medium">{video.createdAt.toDateString()}</span>
                    </div>
            ...
            <Dialog open={showDeleteSureDialog} onOpenChange={setShowDeleteSureDialog}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <div className="flex items-center gap-3 mb-2">
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="grid gap-3 py-4">
                        {['mp4', 'webm', 'mov', 'gif'].map((format) => (
                            <button
                                key={format}
                                onClick={() => setSelectedFormat(format as 'mp4' | 'webm' | 'mov' | 'gif')}
