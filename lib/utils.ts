...
export async function toggleCamStateInStore(isCurrentCamOn: boolean, tabId: number) {
...
export async function checkRocordingStates(): Promise<RecordingState> {
  const isRecording = await storage.getItem<boolean>('local:isRecording') || false;
  const isCamEnable = await storage.getItem<boolean>('local:isCamreRaOn') || false;
...
export async function getRecordingVideoid(){
  const videoId = await storage.getItem<string>("local:recordingVideoId")||null;
  return videoId;
}
...
export async function getUserMediaPermissions() {
    ...
    if (permissions.state === 'denied') {
        alert('Camera permissions denied. Please enable them in your browser settings.');
