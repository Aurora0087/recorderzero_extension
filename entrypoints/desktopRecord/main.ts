...
browser.runtime.onMessage.addListener(async (message, sender, _) => {
  ...
  switch (message.type) {
    ...
    case "stop-recording-desktop":
      ...
      stopRecording(message.pinnedTabId);
      break;
    default:
      break;
  }
...
// recive message from offscreen page
browser.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  switch (message.type) {
    case "save-chunk-onlocaldb":
      console.log("Message recive in worker.");

      if (!message.videoId || !message.data || !message.chunkId) {
        console.warn("VideoId or Cunk Data not given.");
        break;
      }
      await db.chanks.add({
        id: message.chunkId,
...
async function stopRecording(pinnedTabId: number) {
